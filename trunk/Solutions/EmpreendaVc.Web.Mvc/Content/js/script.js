$('document').ready(function () {
    ...
    $('.datepicker').after('<img class="datepickericon" src="../../Content/images/icons/calendar.png" alt="calendar" />');
...
//    });
    ...
    $('.confirm > a').click(function () {
        var CurrentTitle = $('.confirmation').attr('title');
        $('.tooltip').fadeOut(200, function () { // Remove all tooltips
            $(this).remove();
        });
        var titulo = "Apagar Registro?";
        //        if (CurrentTitle != null) {
        //            titulo = CurrentTitle;
        //        }
        var link = $(this).parent().children("a").attr('href'); // Get the requested link
        $(this).parent().append('<div class="tooltip"><p>' + titulo + '</p><a href="' + link + '">Sim</a> | <a href="#" class="close">N&atilde;o</a></div>'); // Adding the tooltip to the DOM
    ...
    $('table.pagination').each(function () {
        ...
        $(this).find(".pagination").append('<a href="#first" rel="first">Primeiro</a>');

        for (var i = 0; i < pageCount; i++) {
            $(this).find(".pagination").append('<a href="#" class="graybutton pagelink" rel="' + (i + 1) + '">' + (i + 1) + '</a>');
        }

        $(this).find(".pagination").append('<a href="#last" rel="last">&Uacute;ltimo</a>');
